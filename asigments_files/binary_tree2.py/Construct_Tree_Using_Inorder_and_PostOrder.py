...
def buildTree(postOrder, inOrder, n) :
    ...
    indexPP = inOrder.index(rootData)

    if indexPP == -1 :
        return None

    lefIn = inOrder[0:indexPP]
    length = len(lefIn)
    leftPost = postOrder[0:length]
    rightIn = inOrder[indexPP+1:]
    rightPost = postOrder[length:-1]

    leftChild = buildTree(leftPost, lefIn, n)
    rightChild = buildTree(rightPost, rightIn, n)
