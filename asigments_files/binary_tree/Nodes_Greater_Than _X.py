...
def countNodesGreaterThanX(root, x, count) :
    ...
    left, count = countNodesGreaterThanX(root.left,x, count)
    right, count = countNodesGreaterThanX(root.right,x, count)
    if left != None and left > x :
        count += 1
    if right != None and right > x :
        count += 1
...
rootData, count = countNodesGreaterThanX(root, x, 0)
if rootData > x:
    count += 1
print(count)
