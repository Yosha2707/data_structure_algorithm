...
def appendLastNToFirst(head, pos, initial, prev) :
    count = 1
    newHead = head
    lastHead = head
    while head is not None:
...
while t > 0 :
    ...
    lengthLinked = length(head)
	
    if n == 0:
        printLinkedList(head)
    elif lengthLinked >= n:
        realPos =  lengthLinked - n
        head = appendLastNToFirst(head, realPos + 1, head, None)
        printLinkedList(head)
